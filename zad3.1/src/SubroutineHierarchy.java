...
public class SubroutineHierarchy extends JPanel {
    ...
    private void drawWorld(Graphics2D g2) {
        ...
        Figure(g2,0,0,0,0);
        Figure(g2,-3.35,2.45,0.003,0.003);
        Figure(g2,-1.6,1.95,0.003,0.003);
        Rect(g2,-2.5,2.2,2,0.2, Color.RED);
        Triangle(g2,-2.5,0.7,0.7,1.5 ,new Color(128,0,128));

        Figure(g2,3.3,2.2,0.002,0.002);
        Figure(g2,1.7,2.6,0.002,0.002);
        Rect(g2,2.5,2.4,1.8,0.2, Color.RED);
        Triangle(g2,2.5,1.1,0.6,1.3 ,new Color(34,139,34));

        Figure(g2,-0.95,0.05,0.004,0.004);
        Figure(g2,0.95,-0.50,0.004,0.004);
        Rect(g2,0,-0.2,2.2,0.3, Color.RED);
        Triangle(g2,0,-2.2,1,2 ,new Color(0,0,255));
    ...
    private void Triangle(Graphics2D g2,double posX, double posY,double scaleX, double scaleY, Color color) {
        AffineTransform saveTransform = g2.getTransform();
        Color saveColor = g2.getColor();
        g2.setColor( color );
    ...
    private void Rect(Graphics2D g2, double posX, double posY,double scaleX, double scaleY, Color color) {
        AffineTransform saveTransform = g2.getTransform();
        Color saveColor = g2.getColor();
        g2.setColor( color );
    ...
    private static void line(Graphics2D g2) { // Draws a line from (-0.5,0) to (0.5,0)
        g2.draw( new Line2D.Double( 0,0, 100,0) );
    }
    ...
    private static void polygon(Graphics2D g2)
    {
        ...
        for (int i = 0; i < n; i++) {
            ...
            g2.draw( new Line2D.Double( 0,0, (int) (150 * Math.cos(i * 2 * Math.PI / n)),
                    (int) (150 * Math.sin(i * 2 * Math.PI / n))) );
        }
